...
*** Org Babel - ob-http and ob-restclient
   ...
   #+begin_src restclient :var url="https://httpbin.org" :wrap src json
     POST :url/post
     Content-Type: application/json
     { "key": "value" }
   #+end_src
   ...
   #+begin_src http :pretty :var pass=(getenv "thepass") :var payload=poodle
   ...
   #+begin_src restclient :var pass=(getenv "thepass") :var payload=poodle
     POST https://api.openai.com/v1/chat/completions
     Content-Type: application/json
     Authorization: Bearer :pass
     {
